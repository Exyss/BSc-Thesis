...
\begin{figure}[H]
    ...
    \caption{The idea behind \Cref{main_thm}}
\end{figure}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[->,>=stealth,shorten >=1pt,auto,node distance=1.25cm, thick,main node/.style={scale=0.9,circle,draw,font=\sffamily\normalsize}]
        ...
        \node (2) [below of = 1]{\textsf{TFNP}$^{dt}$};

        \node (3) [below of = 2]{\textsf{PPP}$^{dt}$};

        \node (4) [left of = 3, xshift=-30]{\textsf{PLS}$^{dt}$};

        \node (5) [right of = 3, xshift=30]{\textsf{PPA}$^{dt}$};

        \node (6) [below of = 3]{\textsf{PPADS}$^{dt}$};

        \node (7) [below of = 6]{};

        \node (8) [left of = 7, xshift=-30]{\textsf{SOPL}$^{dt}$};

        \node (9) [right of = 7, xshift=30]{\textsf{PPAD}$^{dt}$};

        \node (10) [below of = 7]{\textsf{CLS}$^{dt}$};

        \node (13) [right of = 10, xshift=30]{};

        \node (11) [below of = 13]{\textsf{FP}$^{dt}$};
    
        \node (12) [right of = 9, xshift=100]{\textsf{FP}$^{pdt}$};
        ...
        \path[every node/.style={font=\sffamily\small}]
            ...
            (4) edge[dashed, near end] node[yshift=10, xshift=-40]{\Cref{pls_not_inside_fp_pdt}}(12) 
            (11) edge[] node{\Cref{fp_dt_inside_fp_pdt}}(12) 
            (12) edge[bend right, swap] node{\Cref{main_thm}}(5) 
            (12) edge[dashed, bend left] node{\Cref{fp_pdt_not_inside_fp_dt}}(11) 
            ;
    \end{tikzpicture}
