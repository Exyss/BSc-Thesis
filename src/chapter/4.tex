...
The difference in notation between $T_1, \ldots, T_n$ and $T_y^o$ underlines the fact that the former return a $\{0,1\}$ output, while the latter return an output in $O_n$. The \textit{size} of the reduction is the number of input bits to $S$, that being $n$. The \textit{depth} $d$ of the reduction is the maximum depth of any tree involved in the reduction, including each $T_i$ and each $T_y^o$. The complexity of the reduction is denoted as $S(R)$  and it's equal $\log n + d$. A decision tree reduction is considered efficient if its complexity is polylogarithmic, i.e. equal to $O(\log^k n)$. 
...
\begin{proposition}
    Given a CNF formula $F$, it holds that $F \in \mathrm{TAUT}$ if and only if $\lnot F$ is refuted by Resolution
\end{proposition}

For example, given the following unsatisfiable CNF formula $(y \lor z) \land (y \lor \lnot{z}) \land (x \lor \lnot{y} \lor z) \land (\lnot{x} \lor z) \land \lnot{z}$, a Resolution proof is given by:
\[\begin{array}{lcl}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[-,>=stealth,shorten >=1pt,auto,node distance=2.25cm, thick,main node/.style={scale=0.9,circle,draw,font=\sffamily\normalsize}]
        ...
        \node (9) [above right of=6]{$\lnot{x} \lor z$};
...
where $\mathrm{size}(\Pi)$ is the sum of the sizes of all the formulas inside it, or in other words the total number of symbols in $\Pi$, and $\mathrm{deg}(\Pi)$ is the \textit{degree} of $\Pi$ associated to $P$, which varies from proof system to proof system. For example, in Tree-like Resolution the degree is the depth of the proof tree. This degree measure will be specified for the proof systems analyzed in following sections.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[->,>=stealth,shorten >=1pt,auto,node distance=2cm, thick,main node/.style={scale=0.9,circle,draw,font=\sffamily\normalsize}]
        ...
        \node (3) [below of = 2]{\textsf{PPP}$^{dt}$};

        \node (4) [left of = 3, xshift=-40]{\textsf{PLS}$^{dt}$};

        \node (5) [right of = 3, xshift=40]{\textsf{PPA}$^{dt}$};

        \node (6) [below of = 3]{\textsf{PPADS}$^{dt}$};

        \node (7) [below of = 6]{};

        \node (8) [left of = 7, xshift=-40]{\textsf{SOPL}$^{dt}$};

        \node (9) [right of = 7, xshift=40]{\textsf{PPAD}$^{dt}$};

        \node (10) [below of = 7]{\textsf{CLS}$^{dt}$};

        \node (11) [below of = 10]{\textsf{FP}$^{dt}$};

        \node (12) [left of = 4, xshift = 25]{\textsf{Res} $=$};

        \node (13) [left of = 6, xshift = 5]{\textsf{unary-NS} $=$};
        
        \node (14) [left of = 8, xshift = 12.5]{\textsf{RevRes} $=$};
        
        \node (15) [left of = 10, xshift = 12.5]{\textsf{RevResT} $=$};

        \node (16) [right of = 5, xshift = -17.5]{$=$ \textsf{$\F_2$-NS}};

        \node (17) [right of = 9, xshift = -7.5]{$=$ \textsf{unary-SA}};
